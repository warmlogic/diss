...
responses.  For analysis, EEG is typically averaged across brief
...
Similarly, decomposing EEG into time--frequency measures relative to
stimulus onset using methods such as wavelet transforms or multitapers
...
cognitive processes.  Power is typically measured as a change relative to a pre-stimulus baseline, so when a frequency band increases in power (synchronizes) or decreases in power (desynchronizes), this simply means that the amplitude in that frequency band was higher or lower compared to the baseline.
...
\section{Goal 1: Effects of attention and semantic processing}
...
The N400 ERP component (negative central peak) is associated with the processing of meaning, and is affected by semantic processing and priming \cite{KutaFede2011}.  Semantic unexpectedness will produce a more negative N400 because different semantic information becomes activated \cite{KutaHill1980}, but the component is not solely linked to anomalies.  In general, the N400 is thought to reflect the dynamic construction of semantic meaning as part of a larger distributed system (as opposed to being a direct measure of accessing the meaning of a stimulus).  N400 effects are also seen in repetition paradigms where the component attenuates to repetition events compared to initial presentations \cite{KimEtal2001,OlicEtal2000,VanSEtal2007}; voltages becomes more negative as lag between repetitions increases.  The amount the component attenuates to a stimulus repetition or to a target stimulus following a priming stimulus reveals ``how much of the information normally elicited by that stimulus is already active'' \cite[p.~23]{KutaFede2011}.  Spaced repetitions elicit an N400 (and thus semantic activation processes) whereas this does not occur for massed repetitions since the information is still in working memory \cite{VanSEtal2007}.  We would expect this repetition attenuation to be a factor under deficient processing since failing to receive additional semantic processing events is one way subsequent memory could be worse for these items, \hl{(but would also simply expect this to occur for any immediate repetition)}.
...
\section{Goal 2: Effects of memory retrieval and encoding}
...
\section{Goal 3: Memory reinstatement and contextual variability assessed via neural similarity}
