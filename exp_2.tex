...
Perhaps the decreased similarity comes from extraneous information is being activated during massed presentations, thereby making them more variable.  This could align with the stimuli being thought about in a different way, which was proposed regarding the theta band negative SME (main effect).
...
For the similarity of time--frequency data, it makes sense that longer lags would have lower similarity because context has drifted for these repetitions more than for short lags.  The remaining effects seems to neither support nor challenge the hypotheses.

% we did not see the three-way interaction as in Experiment~1.
...
Behaviorally, we saw exactly what was expected: performance increased significantly as lag increased.  The EEG effects mostly followed the patterns of Experiment~1.  TODO The main differences were


Across the ERP results, it seems like information that is already primed and in working memory and does not need to be retrieved (massed) does not gain the repetition advantage of information that needs to be retrieved (spaced).
