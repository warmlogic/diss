...
If selective attention is modulated by spacing, early ERP components may show effects; this is relevant to deficient processing.  However, there were no significant effects for N1 voltage in the full ANOVA. \cbstart  Because of \textit{a priori} interest based on the results from Experiment~1, we examined the pairwise comparisons in the three-way interaction, which was significant in Experiment~1.  Recalled medium (12) spaced repetitions were marginally more negative ($M=-0.27~\mu$V) than recalled massed repetitions ($M=0.03~\mu$V) [$t(26)=1.99, p=.057$].  The difference was also marginal for recalled spaced items ($M=-0.26~\mu$V) [$t(26)=2.04, p=.052$] but not nearly so marginal for long (32) spaced items ($M=-0.27~\mu$V) [$t(26)=-1.26, p=0.22$], and forgotten conditions were all slightly attenuated.  This pattern of results is similar to Experiment~1.\cbend
...
The lack of significant N1 effects leads to the idea that attention is not the critical factor for why the spacing effect occurs, at least when defining attention as an early involuntary mechanism indexed by the N1, though it should be kept in mind that the overall pattern from Experiment~1 persisted.  We cannot make any strong claims in the face of null results, but overall this is less evidence to support deficient processing.
There are a number of possibilities to consider for the attenuated N1 effect.

Perhaps an experiment difference such as removing the exposure phase made a difference for the N1 (e.g.,~having an existing representation of a stimulus before needing to learn stimulus pairings could change attentional mechanisms).  Regardless, removing the exposure phase still led to a strong spacing effect in Experiment~2.

Another possibility to consider is that spaced repetitions were relatively more likely to occur in the second experiment (approximately 75\% of the time) compared to Experiment~1 (approximately 50\% of the time).  Perhaps this made spaced repetitions less attention grabbing.  \hl{(TODO: Have stimulus probability effects been documented for the N1?)}
...
Second, the gradation might go in the other direction: effects for working memory should be stronger for massed items and decrease with lag.  This larger difference for subsequent memory of massed items is what we see; the overall results showed that massed repetitions were more positive and peaked earlier than spaced repetitions.
...
Though we have found some support for deficient processing, specifically in relation to massed items, the mechanisms involved in this hypothesis seem to have little bearing on any spaced condition and therefore cannot capture the lag effect.  Thus, we still have not found a defining neural signature for why performance increases with lag.
