...
\hl{A lot of the following text has been repeated below, would probably be best to shorten this intro and move more of it to discussion. Fill out discussion interleaved with each results section first.}

% if commented, has been used below, can delete
% With more inter-study lags, we would expect deficient processing to show a graded effect, specifically decreasing with lag.  If this process is occurring as we've defined it, the N1 should become more negative as lag increases because early attentional processes vary with repetition lag; a repetition at lag 2 would have a voltage between a massed item and a repetition at lag 12.

% We would expect semantic processes (N400 and upper alpha) as well as working memory effects that vary with repetition lag (LPC) to show similar gradients: N400 should get more negative as lag increases and upper alpha for remembered spaced items should desynchronize more as lag increases.

% An important question to consider regarding the N1 is how short term the repetition effects are.  According to \citeA{HensEtal2004}, who investigated ERP effects for repetitions of pictures of objects at different lags, they saw an N1 repetition effect at after an unfilled 4-second delay, but not when the four seconds was occupied by another stimulus or at a much longer lag (96 seconds).  Thus, deficient processing may be eliminated at a relatively short delay.
...
It would be enlightening to get away from a paradigm that involves immediate repetition so that deficient processing could be eliminated as a possible theory.  Perhaps a lag effect between spacings of 7 and 36 items would be spaced apart far enough to eliminate deficient processing.

\hl{End of text that has been replicated below.}
...
For the N400, we expected semantic processing to show graded effects: it would get more negative as lag increases because more semantic processing is needed.  There were no graded effects within spaced conditions, only a difference between massed and spaced items in the expected direction (massed items were strongly attenuated compared to spaced).  The means (Figure~\ref{fig:s2_N400}e) show an overall decrease for remembered items as lag increases, but there was no significant effect of memory (as in Experiment~1).  Given that the N400 is tied to semantic processing, it seems that a similar amount of processing is engaged when there have been any number of intervening stimuli between repetitions.  This would be in line with a deficient processing theory that only posits deficient processing for immediate repetitions; however, behavioral performance does not mimic this patter.
...
% why would SME decrease with lag?


We would expect working memory effects (LPC) to be graded across lags.  This could go in two direction.  First, for working memory, effects should be stronger for massed items and decrease with lag; this larger difference for subsequent memory of massed items is what we see.  \hl{what does this support?}  The other hypothesis is in terms of memory retrieval (posited under study-phase retrieval), which we might see as a subsequent memory effect.  Reinstatement should be more difficult as lag increases (as described in \hl{these} models) but also more beneficial to long-term memory.


It again seems possible that the LPC indexes the information that is in working memory.  This result does not seem to directly support any of the theories, unless the match to working memory is an indicator for deficient processing to engage.

As exhibited through behavioral performance, spaced items were obviously not forgotten even though they did not show an LPC memory effect, so this effect must not be indexing recollection (this makes sense when comparing its topography to that of the parietal old/new effect, typically more left parietal).
...
We still have not found the defining neural signature for why long spaced performance is best.
...
% words
\textit{Word, theta}: Across 73 electrodes (Figure~\ref{fig:s2_word_img_theta}, top), there were main effects of spacing [$F(3,78)=4.0, p<.05$] and latency [$F(1,26)=33, p=4.81e^{-6}$].  Long spaced word repetitions showed more theta power than all other conditions [$p$s~$<.05$], and there was a decrease in power across time.  There was also a spacing $\times$ latency interaction [$F(3,78)=16, p=5.33e^{-8}$] such that all conditions decreased across time [$p$s~$<.05$] except long spaced words.

There was no three-way interaction, but pairwise comparisons between recalled words across massed and spaced conditions in the later time window showed that long spaced words had significantly higher theta power than all other conditions.  This is interesting because this category of trials had the highest behavioral performance; in fact, the pattern of theta power for recalled repetitions in the second time window follows behavioral performance.

\textit{Image, theta}: Across 90 electrodes (Figure~\ref{fig:s2_word_img_theta}, bottom), there were main effects of spacing [$F(3,78)=27.2, p=1.17e^{-10}$], memory [$F(1,26)=8.76, p<.01$], and latency [$F(1,26)=91.1, p=5.57e^{-10}$].  Massed items had more desynchrony than the other conditions, subsequently recalled items had more desynchronization than forgotten ones, and power decreased across time.  A spacing $\times$ latency interaction showed that the decrease in power across time for massed items was less than short and medium spaced repetitions.


\hl{Theta is the other place (in addition to the LPC) where we would expect to see effects of reinstatement.  We would expect subsequently remembered long-lag stimuli to show a stronger reinstatement during repetitions.}
...
\textit{Word, lower alpha}: Across 49 electrodes (Figure~\ref{fig:s2_word_img_alpha_low}, top), there were significant main effects of spacing [$F(3,78)=9.78, p=2.22e^{-5}$], memory [$F(1,26)=5.89, p<.05$], and latency [$F(1,26)=9.71, p<.005$], as well as the spacing $\times$ latency interaction [$F(3,78)=15.1, p=2.15e^{-7}$].  Massed word repetitions showed significantly lower alpha desynchronization than the other conditions, there was more desynchronization for recalled images, there was more desynchronization in the second time window, and the decrease in the second time window was larger for massed words.


\textit{Image, lower alpha}: Across 94 electrodes (Figure~\ref{fig:s2_word_img_alpha_low}, bottom), lower alpha showed the same pattern of effects theta, with the addition of a memory $\times$ latency interaction [$F(1,26)=5.37, p<.05$].  Recalled items decreased more across time than forgotten ones.
...
\textit{Word, upper alpha}: Keeping in line with Experiment~1, we used three time windows for upper alpha and lower beta.  Across 46 electrodes (Figure~\ref{fig:s2_word_img_alpha_upp}, top), there was a main effect of latency [$F(2,52)=18.3, p=3.19e^{-6}$], a spacing $\times$ latency interaction [$F(6,156)=11.8, p=7.51e^{-10}$], and a marginal three-way interaction [$F(6,156)=2.14, p=.052$].  The second and third time windows had more desynchronization than the first.  Each repetition decreased in power across time, but massed decreases more.  For the three-way interaction, recalled items tended to stay desynchronized in the third time window while forgotten items increased slightly in power, and this increase seemed to happen more for short and long spaced items.

\hl{From intro: Like the N400 ERP component, we would expect semantic processing (upper alpha) to show graded effects: upper alpha for remembered spaced items should desynchronize more as lag increases.}


\textit{Image, upper alpha}: Across 44 electrodes (Figure~\ref{fig:s2_word_img_alpha_upp}, bottom), there were main effects of memory [$F(1,26)=21.8, p=8.08e^{-5}$] and latency [$F(2,52)=25, p=5.67e^{-8}$] (spacing was marginal, massed was only significantly lower than short and medium spaced items).  Recalled had more desynchronization than forgotten items, and power was lowest in the middle time window.  There was also a memory $\times$ latency interaction [$F(2,52)=6.82, p<.005$] where remembered items continued to stay desynchronized into the third time window.
...
\textit{Word, lower beta}: Across 66 electrodes (Figure~\ref{fig:s2_word_img_beta_low}, top), there were main effects of spacing [$F(3,78)=9.48, p=7.16e^{-5}$], memory [$F(1,26)=8.36, p<.01$], latency [$F(2,52)=25.5, p=4.0e^{-8}$], a spacing $\times$ latency interaction [$F(6,156)=6.96, p=3.79e^{-6}$], and a marginal three-way interaction [$F(6,156)=2.02, p=0.068$].  Massed showed more lower beta desynchronization than the spaced conditions, subsequently recalled words had more beta desynchronization than forgotten ones, and there was more desynchronization during the second time window compared to the others though the third was still lower than the first.  Additionally, massed items decreased more across time and stayed decreased throughout the trial compared to spaced items.  The marginal three-way interaction showed the same pattern as upper alpha: recalled items tended to stay desynchronized in the third time window while forgotten items increased slightly in power, and this increase seemed to happen more for short and long spaced items.


\textit{Image, lower beta}: Across 30 electrodes (Figure~\ref{fig:s2_word_img_beta_low}, bottom), there were main effects of memory [$F(1,26)=25.2, p=3.22e^{-5}$] and latency [$F(2,52)=53.7, p=3.33e^{-12}$] which followed the patterns of effects reported above (recalled had more desynchronization and power was lowest in the second time window and rose back up in the third).


\subsubsection{Time--frequency discussion}

\hl{From intro repeated: At longer lags, study-phase retrieval should be more difficult but also more beneficial to long-term memory.  We would expect subsequently remembered long-lag stimuli to show a stronger reinstatement during repetitions.  This would manifest in the theta band, and perhaps would also show stronger recollection memory ERP effects (such as the parietal old/new effect).}

Theta power for long spaced items showed an effect, meaning that the retrieval of long spaced items occurred more compared to the other conditions.  Theta power during recalled long spaced word repetitions showed even greater levels compared to the other conditions, meaning retrieval occurred.  This is in line with the \citeA{Gree1989a} idea (and some models--- \citeA{PavlAnde2005} and \citeA{MozeEtal2009}???) that more difficult retrievals (due to increased spacing) leads to better memory. Though the base rate for long spaced items was higher overall, doesn't mean there were fewer strong trials?

It is interesting to contrast increased theta for long spaced items with the ERP LPC effects where massed items showed the most retrieval.  As was mentioned before, the LPC in this task may be related to primed information and may not necessarily reflect recollection.
...
For time--frequency data, the same three-way ANOVA was performed over left and right parietal regions across all frequency bands.
There was a main effect of spacing [$F(3,78)=8.92, p=6.5e^{-5}$] (similarity decreased with lag), a main effect of latency [$F(4,104)=2.9, p<.05$] (similarity increased across time), and a marginal memory $\times$ latency interaction [$F(4,104)=2.46, p=0.051$] (without a very clear pattern).
...
\hl{From exp 2 intro: Again, the purpose of this experiment is to look for gradations in EEG effects so we can better interpret data patterns that fit multiple theories.}
