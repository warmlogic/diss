...
This implies that qualitatively similar attentional processing patterns are showing here, as compared to Experiment~1.
...
This implies that the more negative spaced trials receive more semantic processing, with subsequently recalled medium spaced trials getting more processing than those that are forgotten.
...
This implies that it takes longer to access medium and long spaced representations compared to massed and short spaced conditions.
...
It again seems that the LPC indexes the information that is in working memory, and again, the result does not seem to directly support any of the theories unless the match to working memory for massed items is an indicator for processing to disengage (deficient processing).
...
For time--frequency data (plotted in Figure~\ref{fig:s2_sim_pow_spacXmemXtime}), the same three-way ANOVA was performed over left and right parietal regions across all frequency bands.
