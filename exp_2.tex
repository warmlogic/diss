...
Finally, we would expect working memory effects (LPC) to be graded across lags.  This could by for two reasons, in opposite directions.  First, in terms of memory retrieval (posited under study-phase retrieval), which we might see as a subsequent memory effect.  Reinstatement should be more difficult as lag increases (as described in the models of \citeNP{PavlAnde2005} and \citeNP{MozeEtal2009}) but also more beneficial to long-term memory if it is successful.
...
There was no three-way interaction, but pairwise comparisons between only recalled words across massed and spaced conditions in the later time window showed that long spaced words had significantly higher theta power than all other conditions.
...
As discussed previously, lower alpha should correlate negatively with general attentional processes with a widespread scalp topography.  Deficient processing would predict increased alpha power for massed repetitions.  We would also expect an effect of memory such that decreased power (more desynchronization) would be associated with better subsequent memory.

Lower alpha results followed Experiment~1: we see the memory effect but not the spacing effect in the expected direction.  Again, more attention seems to get allocated to massed items.  It is possible that these effects are related to orientation during working memory.
...
In relation to spacing, like the N400 ERP component, we expected semantic processing (denoted by upper alpha) to show graded effects: upper alpha for remembered items should desynchronize more as lag increases because items with longer lag are remembered better on average, and semantic processing should help memory.  These results show that remembered items incur more semantic processing across the trial, but no spacing effect.  With respect to massed items showing more semantic processing, perhaps the effect is driven by the semantic representation still being active in working memory.  As in Experiment~1, these results do not support deficient processing.
...
Theta is the other place (in addition to the LPC) where we would expect to see effects of reinstatement.  We would expect subsequently remembered long-lag stimuli to show stronger reinstatement during repetitions.  The sustained theta effect for long spaced word repetitions (including pairwise comparisons for only recalled items) is interesting because this category of trials had the highest behavioral performance; in fact, the pattern of theta power for recalled word repetitions in the second time window follows behavioral performance.
