...
% LAG EFFECTS
Experiment~2 also showed a voltage lag effect.  SMEs (recalled \textit{vs.} forgotten) decrease with lag, likely indicating that less information is recognized at longer lags, which is in line with spacing effect research involving long lags \cite<e.g.,>{Glen1979}.
...
It seems that the characterization of deficient processing as an ``impostor'' spacing effect by \citeA{DelaEtal2010} is a somewhat accurate label: the pattern of results supporting deficient processing explain what makes massed items less memorable rather than what makes spaced items more memorable due to differentiating massed from any spaced condition and not generally showing lag effects.
% Thus, this theory accounts for the spacing effect with a relative difference between the two conditions, and
Because of its reliance on the attenuation of attentional mechanisms and semantic processing, which would likely fade after a brief period of time, it only seems like a viable explanation for the spacing effect at short timescales.
