...
The spacing effect shows that longer lags between repetitions produces better subsequent memory performance\footnote{In reality the retention interval between the final study event and the test event will also have an impact on the optimal lag \cite{Glen1979}; optimal lag will not be considered here.}.

As lag increases between repetitions it may become more difficult to retrieve the prior study event, but if a longer lag item is retrieved then the memory trace is strengthened to a greater extent \cite{DelaEtal2010}.  The reason behind this is not specified, but perhaps

context will have drifted sufficiently far to make recognition difficult, and the larger contextual difference will provide more cues for later retrieval.

Some of the memory models described below also describe a possibility \cite{MozeEtal2009,PavlAnde2005}.  If an item has high activation when it is repeated (as would be expected for a massed item) then it will begin to decay quickly at that point, whereas low activation at a repetition (for a spaced item) would lead to slower decay of the trace.

Notably, this retrieval difficulty idea is in line with research on the testing effect \cite<e.g.,>{KarpRoed2007}. \hl{(TODO: AH asked: how is it in line?)}
