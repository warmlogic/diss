...
As lag increases between repetitions it may become more difficult to retrieve the prior study event, but if a longer lag item is retrieved then the memory trace is strengthened to a greater extent \cite{DelaEtal2010}.  The processes underlying this is not specified in the literature, but this is essentially a description of the spacing effect and the mechanisms involved are part of the investigations performed in this thesis.
...
Notably, this retrieval difficulty idea is in line with research on the testing effect.  For example, \citeA{KarpRoed2007} showed that a more difficult retrieval due to showing a repetition at a longer delay promoted long-term retention (here, the retrieval at repetition was a test).
